...
public class Appium_reportgenerator {

	public void report() throws IOException, RowsExceededException, WriteException, BiffException{
		//DateFormat logTme = new SimpleDateFormat("hh:mm:ss");
		Calendar cal = Calendar.getInstance();
		//String logTime = logTme.format(cal.getTime());		
		DateFormat dateFormat = new SimpleDateFormat("MM_dd_yyyy");		
		String cal1 = dateFormat.format(cal.getTime());
		String filepath = "C:\\Appium\\Results\\"+Appium_appselection.selectedApp+" Final Report "+cal1+".xls";
		File ifilepath = new File("C:\\Appium\\Results\\"+Appium_appselection.selectedApp+" Final Report "+cal1+".xls");
		String ofilepath = "C:\\Appium\\Results\\"+Appium_appselection.selectedApp+" Final Report "+cal1+"_temp.xls";
		...
		if (!logfile.exists()){			//if1
		    ...
		    sheet.addCell(new Label(4, 0, "Error Type",cellFormat));
		    workbook.write(); 
		    workbook.close();
		} //if1 ends
	    ...
	    widthInChars = 16;
	    sheet1.setColumnView(col, widthInChars);
	    sheet1.addCell(new Label(col, newrow,Appium_driverscript.crntSheet,cellFormat1));
	    col = 2;
	    widthInChars = 70;
	    sheet1.setColumnView(col, widthInChars);
	    if (Appium_driverscript.freport) {
	    sheet1.addCell(new Label(col, newrow,Appium_driverscript.crntStep,cellFormat1));
	    Appium_driverscript.freport = false;
	    }
	    else {
	    sheet1.addCell(new Label(col, newrow,Appium_driverscript.Module+" - " +Appium_driverscript.ElementName,cellFormat1));
	    }
	    col = 3;
	    widthInChars = 15;
	    sheet1.setColumnView(col, widthInChars);
	   
	    if (Appium_driverscript.Teststatus=="PASS") { //if2
	    	sheet1.addCell(new Label(col, newrow,Appium_driverscript.Teststatus,passcellFormat));
	    	col = 4;
	 	    widthInChars = 20;
	 	    sheet1.setColumnView(col, widthInChars);
	    	sheet1.addCell(new Label(col, newrow,"",cellFormat1));
	    	Appium_driverscript.ErrDesc = "";
	    }else{
	    	sheet1.addCell(new Label(col, newrow,Appium_driverscript.Teststatus,failcellFormat));	   
	    	col = 4;
	 	    widthInChars = 20;
	 	    sheet1.setColumnView(col, widthInChars);
		    sheet1.addCell(new Label(col, newrow,Appium_driverscript.errorcriticality,cellFormat1));
	    } //if2 ends
	   /* col = 4;
	    widthInChars = 50;
	    sheet1.setColumnView(col, widthInChars);
	    sheet1.addCell(new Label(col, newrow,Appium_driverscript.ErrDesc,cellFormat1));*/
